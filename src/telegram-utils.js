...
module.exports.sprom = function (data) {
    return new Promise(function (resolve) {
        resolve(data);
    });
};

module.exports.enableTelegramControllers = function (services) {
                ...
                switch (action.type) {
                    case 'command':
                        bot.onText(action.pattern, function (msg) {
                            ...
                            const from = msg.from;
                            db
                                .checkUser(from.id, from.username, from.first_name, from.last_name)
                                .then(function (user) {
                                    if (action.role && !user.dataValues.hasRole(action.role)) {
                                        return;
                                    }

                                    updateKeyboard(user, msg, lastKeyboardUpdate, handleResponse);

                                    return action.handler({user: user.dataValues, msg: msg});
                                })
        ...
        bot.on('callback_query', function (msg) {
            ...
            db
                ...
                .then(function (user) {
                    ...
                    return action.handler({user: user.dataValues, msg: msg, parts: parts});
                })
                .then(handleResponse(msg))
            ;
        ...
        bot.on('successful_payment', function (msg) {
            ...
            db
                ...
                .then(function (user) {
                    ...
                    return action.handler({user: user.dataValues, msg: msg, payment: payment});
                })
                .then(handleResponse(msg))
            ;
        });
            ...
            db
                ...
                .then(function (user) {
                    ...
                    const promises = _.map(serviceQueries.contact, function (action) {
                        ...
                        return action.handler({user: user.dataValues, msg: msg, contact: contact});
                    });
            ...
            db
                ...
                .then(function (user) {
                    ...
                    const promises = _.map(serviceQueries.text, function (action) {
                        ...
                        return action.handler({user: user.dataValues, msg: msg, text: text});
                    });
            ...
            db
                ...
                .then(function (user) {
                    ...
                    const promises = _.map(serviceQueries.photo, function (action) {
                        ...
                        return action.handler({user: user.dataValues, msg: msg, photo: photo});
                    });
...
const updateKeyboard = function (user, msg, lastKeyboardUpdate, handleResponse) {
    console.log('check time', user.dataValues.last_activity_at, lastKeyboardUpdate, new Date);
    if (user.dataValues.last_activity_at.getTime() < lastKeyboardUpdate.getTime()) {
